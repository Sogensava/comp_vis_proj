...
def plot_gaussian_curve(mean=0, std=1, title=None, save_path=None):
    """
    Plot a Normal Gaussian Curve with specified mean and standard deviation.

    Parameters:
        mean (float): Mean (\mu) of the Gaussian distribution.
        std (float): Standard deviation (\sigma) of the Gaussian distribution.
        title (str): Title of the plot.
        save_path (str): Path to save the plot (optional).
    """
    # Generate x values
...
def save_subband_video(subband, subband_name, plane,name):
...
def analyze_results(name,subband):
    ...
    for subbands in details:
        print(subbands)
        save_subband_video(details[subbands],subbands,"xy",name)
        save_subband_video(details[subbands],subbands,"xz",name)
        save_subband_video(details[subbands],subbands,"yz",name)
...
for names in name_list:
    append_dict = analyze_results(names,'ddd')
#     features_dict.update(append_dict)
#     print(f'extracted {names}')


# save_features_to_csv(features_dict,'video_features.csv')
